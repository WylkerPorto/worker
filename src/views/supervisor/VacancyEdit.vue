<template>
  <main class="main">
    <section class="card">
      <header>
        <button class="rounded" @click="$router.push({ name: 'vacancyController' })" title="Voltar">
      ...
      <form @submit.prevent="validate">
        ...
        <MyButton class="btn danger" @click="$router.push({ name: 'vacancyController' })">
          Cancelar
        </MyButton>
      </form>
    </section>
...
import { getEmploymentTypes, getWorkModels } from '@/api/filters'
import { list as getPositions } from '@/api/position'
import { get, update } from '@/api/vacancy'
  ...
  methods: {
    ...
    async saveVacancy() {
      ...
      try {
        ...
        this.$router.push({ name: 'vacancyController' })
      } catch (error) {
        this.$snotify.error('Erro ao salvar a vaga: ' + error.response.data.message)
      } finally {
        this.loading = false
      }
    },
