<template>
  <ModalBase :open="show" :loading="loading" title="Confirmar Exclusão" @onClose="$emit('onClose')">
    ...
    <template #actions>
      <MyButton class="btn danger" :loading="loading" @click="deleteDepartment">Confirmar</MyButton>
    </template>
  </ModalBase>
</template>
  ...
  methods: {
    async deleteDepartment() {
      ...
      try {
        ...
        this.$snotify.success('Setor excluído com sucesso!')
        this.$emit('onConfirm')
        this.$emit('onClose')
      } catch (error) {
        this.$snotify.error('Erro ao excluir o setor: ' + error.response.data.message)
      } finally {
        this.loading = false
      }
    },
